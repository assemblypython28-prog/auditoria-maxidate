...
# ============================================================
# CONFIGURAR PATH DO TESSERACT (Linux/Streamlit Cloud)
# ============================================================
try:
    import pytesseract
    ...
    if arquivo is not None:
        try:
            ...
            if df_clean.empty:
                ...
                total = len(df_clean)
                st.info(f"Importando {total} itens...")
...
if df_atual.empty:
    ...
    with tab1:
        ...
        if not TESSERACT_DISPONIVEL:
            st.warning("OCR nao disponivel no servidor. Verifique se o arquivo 'packages.txt' foi criado com 'tesseract-ocr'. Usando entrada manual.")
        else:
                ...
                if TESSERACT_DISPONIVEL:
                    with st.spinner("Analisando imagem com OCR..."):
                        ...
                        if texto_ocr:
                            ...
                            if codigo_detectado:
                                st.markdown('<div class="ocr-box"><h4>Codigo Detectado: <code>' + codigo_detectado + '</code></h4><p>O sistema identificou este codigo na etiqueta!</p></div>', unsafe_allow_html=True)
                                st.session_state.codigo_ocr = codigo_detectado
                            else:
                                st.markdown('<div class="warning-box"><h4>Codigo nao reconhecido automaticamente</h4><p>Nao foi possivel extrair um codigo valido. Digite manualmente abaixo.</p></div>', unsafe_allow_html=True)
                        else:
            ...
            if not item.empty:
                ...
                if status_atual == "Auditado":
                    ...
                    if st.button("CONFIRMAR AUDITORIA", type="primary"):
                        ...
                        sucesso = salvar_item_supabase(codigo_completo, descricao, "Auditado", data_agora, obs, obra_id)
                        if sucesso:
                ...
                st.markdown('<div class="error-box"><h4>Item Nao Encontrado</h4><p>Este codigo nao existe no inventario atual.</p></div>', unsafe_allow_html=True)
                with st.expander("Cadastrar como Sobra"):
                    ...
                    if st.button("Salvar como Sobra"):
                        if desc_sobra:
                            data_agora = pd.Timestamp.now().strftime("%d/%m/%Y %H:%M")
                            salvar_item_supabase(busca, f"[SOBRA] {desc_sobra}", "Auditado", data_agora, obs_sobra, obra_id)
                            st.session_state.db = carregar_do_supabase(obra_id)
